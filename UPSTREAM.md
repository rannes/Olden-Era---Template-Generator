...
| `4ae647c` | auto-update download + UpdateProgressWindow | port (this PR) | Reimplemented under `src/OldenEra.TemplateEditor/Services/AutoUpdate/`. Adds checker / downloader / batch-installer split, logging to `%LOCALAPPDATA%/OldenEraTemplates/update.log`, and a "Check for updates on startup" toggle. Browser fallback preserved. |
